(function ($) {
		...
		$('.drop').on('click', function (e) {
			switch (e.target.innerHTML) {
				...
				case "关于我们":
					let aboutus=$("#AboutUs");					
					$('html, body').animate({
						scrollTop: Math.floor(aboutus.offset().top-79)
					}, 0);
					break;
				case "产品中心":
					let productcenter=$("#ProductCenter");	
					// console.log('jibisheng:',productcenter.offset().top-79);			
					$('html, body').animate({
						scrollTop: Math.floor(productcenter.offset().top-79)
					}, 0);
					break;
				case "合作园区":
					let partnerschool=$("#PartnerSchool");					
					$('html, body').animate({
						scrollTop: Math.floor(partnerschool.offset().top-79)
					}, 0);
					break;
				case "新闻动态":
					let newsactive=$("#NewsActive");					
					$('html, body').animate({
						scrollTop: Math.floor(newsactive.offset().top-79)
					}, 0);
					break;
				case "联系我们":
					let contactus=$("#ContactUs");					
					$('html, body').animate({
						scrollTop: Math.floor(contactus.offset().top-79)
					}, 0);
					break;
				default:
					break;
	...
	//=== 9. 滚动距离控制导航栏当前项
	function ChangeNav(){
		let primary_menu=document.querySelector('.primary-menu')
        let lis=primary_menu.querySelectorAll('li')

		let scroll = $(window).scrollTop();
		
		// console.log(scroll);
		let productcenter_height=Math.floor($("#ProductCenter").offset().top-79);	
		let partnerschool_height=Math.floor($("#PartnerSchool").offset().top-79);
		let aboutus_height=Math.floor($("#AboutUs").offset().top-79);
		let newsactive_height=Math.floor($("#NewsActive").offset().top-79);
		let contactus_height=Math.floor($("#ContactUs").offset().top-79);
		let contactus_actualheight=Math.floor($("#ContactUs")[0].clientHeight);
		for(let i=0;i<lis.length;i++){
			lis[i].className='';
		}
		if(scroll>=(contactus_height-contactus_actualheight)){
			lis[5].className="current"
		}else if(scroll>=newsactive_height&&scroll<(contactus_height-contactus_actualheight)){
			lis[4].className='current'
		}else if(scroll>=aboutus_height){
			lis[3].className='current'
		}else if(scroll>=partnerschool_height){
			lis[2].className='current'
		}else if(scroll>=productcenter_height){
